...
int main(int argc, char *argv[])
{
  ...
  12:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    ...
    for (int i=0;i<6;i++) 
    {
        ...
        {
            for (d1=0;d1<7324567;d1=d1+1) 
  18:	d8 c1                	fadd   %st(1),%st
  1a:	d9 05 34 07 00 00    	flds   0x734
  20:	df e9                	fucomip %st(1),%st
  22:	77 f4                	ja     18 <main+0x18>
  24:	dd d8                	fstp   %st(0)
int main(int argc, char *argv[])
{
    long double d1, d2 = 2.334, d3 = 23.323, d4 = 232.32;
...
int main(int argc, char *argv[])
{
    long double d1, d2 = 2.334, d3 = 23.323, d4 = 232.32;
    for (int i=0;i<6;i++) 
  2b:	83 ea 01             	sub    $0x1,%edx
  2e:	75 d7                	jne    7 <main+0x7>
  30:	dd d8                	fstp   %st(0)
#include "stat.h"
#include "user.h"
#include "fcntl.h"
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 39f:	f7 f6                	div    %esi
 3a1:	0f b6 92 40 07 00 00 	movzbl 0x740(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 518:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 51b:	b8 38 07 00 00       	mov    $0x738,%eax
...
free(void *ap)
{
 ...
 5a1:	a1 d8 09 00 00       	mov    0x9d8,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 5f7:	89 08                	mov    %ecx,(%eax)
  freep = p;
 5f9:	a3 d8 09 00 00       	mov    %eax,0x9d8
}
 5fe:	5b                   	pop    %ebx
 5ff:	5e                   	pop    %esi
 600:	5f                   	pop    %edi
 ...
 61a:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 61d:	a3 d8 09 00 00       	mov    %eax,0x9d8
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 639:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 63c:	8b 15 d8 09 00 00    	mov    0x9d8,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
 ...
 68f:	76 3f                	jbe    6d0 <malloc+0xa0>
    ...
    if(p == freep)
 691:	39 05 d8 09 00 00    	cmp    %eax,0x9d8
 ...
 6b6:	e8 e5 fe ff ff       	call   5a0 <free>
  return freep;
 6bb:	8b 15 d8 09 00 00    	mov    0x9d8,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 6dc:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
 6df:	89 15 d8 09 00 00    	mov    %edx,0x9d8
      return (void*)(p + 1);
 ...
 6f4:	eb e9                	jmp    6df <malloc+0xaf>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 6f6:	c7 05 d8 09 00 00 dc 	movl   $0x9dc,0x9d8
 6fd:	09 00 00 
 700:	c7 05 dc 09 00 00 dc 	movl   $0x9dc,0x9dc
 707:	09 00 00 
    base.s.size = 0;
 70a:	b8 dc 09 00 00       	mov    $0x9dc,%eax
 70f:	c7 05 e0 09 00 00 00 	movl   $0x0,0x9e0
 716:	00 00 00 
